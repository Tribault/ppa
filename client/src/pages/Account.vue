  ...
  <div>
    ...
    <table class="min-w-full table-auto border">
      <thead class="bg-gray-100">
       ...
       <tbody>
      <BookingCard v-for="b in bookings" :key="b._id" :booking="b" :columns="columns"/>
      </tbody>
       </table>
  </div>
    </div>
...
const columns = ref([
  { key: 'poster.title', label: 'Poster' },
  { key: 'quantity', label: 'Quantity' },
  { key: 'status', label: 'Status' },
  { key: 'priceAtBooking', label: 'Price at Booking' },
...
onMounted(async () => {
  try {
    const res = await axios.get(
      `http://localhost:5000/api/bookings/user/${auth.user._id}`,{headers: { Authorization: `Bearer ${auth.token}` }}
  )
