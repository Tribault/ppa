<template>
  <div class="database-search" ref="dropdownRef">
    ...
    />
    <ul
      v-if="searchType == 'user' && showDropdown && userResults.length"
      class="database-search-dropdown"
    >
      <li
        v-for="(u, idx) in userResults"
        :key="u._id"
        @click="sendResult(u, u.email)"
    ...
    <ul
      v-if="searchType == 'poster' && showDropdown && posterResults.length"
      class="database-search-dropdown"
    >
      <li
        v-for="(p, idx) in posterResults"
        :key="p._id"
        @click="sendResult(p, p.title)"
        :class="{ highlighted: idx === highlightedIndex }"
      >
        {{ p.title }}
      </li>
...
function moveHighlight(direction: number) {
  ...
  highlightedIndex.value = (highlightedIndex.value + direction + list.length) % list.length
}
