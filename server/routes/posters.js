...
const {
    getPosters, getPoster, getPosterFilters, checkDuplicateTitle, createPoster, updatePoster, deletePoster
} = require('../controllers/posterController')

router.get('/', optionalAuthenticate, getPosters)
router.get('/filters', getPosterFilters)
router.get('/check-title', authenticate, authorize('admin'), checkDuplicateTitle)
